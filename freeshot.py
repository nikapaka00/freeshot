...
# The hidden Tk root window steals keyboard focus at startup (overrideredirect
# popups don't return focus to the previous app when withdrawn). Setting
# WS_EX_NOACTIVATE on the root prevents it from ever receiving focus, so
# typing goes to whichever real app the user has in front.
_GWL_EXSTYLE      = -20
...
def _set_window_noactivate(hwnd: int, enable: bool) -> None:
    ...
    try:
        ...
        if enable:
            ...
            ex &= ~_WS_EX_NOACTIVATE   # keep WS_EX_TOOLWINDOW — harmless
...
class SelectionOverlay:
    ...
    def __init__(self, root, shot: Image.Image, done_cb, config: Config):
        ...
        self.win.bind("<F>",  lambda _: self._set_mode(self.FREE))
        self.win.update()           # ensure window is fully mapped before grabbing focus
        self.win.lift()
        self.win.focus_force()
        # AttachThreadInput workaround — Windows often blocks SetForegroundWindow
        # for background processes (e.g. tray apps). This sequence always works.
        try:
            _u32   = ctypes.windll.user32
            _hwnd  = self.win.winfo_id()
            _fg    = _u32.GetForegroundWindow()
            _fg_tid = _u32.GetWindowThreadProcessId(_fg, None)
            _our_tid = ctypes.windll.kernel32.GetCurrentThreadId()
            if _fg_tid and _fg_tid != _our_tid:
                _u32.AttachThreadInput(_fg_tid, _our_tid, True)
                _u32.SetForegroundWindow(_hwnd)
                _u32.AttachThreadInput(_fg_tid, _our_tid, False)
            else:
                _u32.SetForegroundWindow(_hwnd)
            _u32.SetFocus(_hwnd)
        except Exception:
...
class FreeShotApp:
    ...
    def _setup_hotkeys(self):
        ...
        We previously used WH_KEYBOARD_LL to intercept PrtScrn, but a
        Python-implemented low-level keyboard hook breaks system-wide
        keyboard input on some Windows 11 configurations — the hook
        callback's GIL-acquisition delay causes the OS to either
        bypass the hook for every key OR (worse) wedge the input
        queue so keys reach no window at all.

        RegisterHotKey is the right tool: it does NOT sit in the
        keyboard input chain. It just tells the OS "post WM_HOTKEY
        when this exact combo is pressed" — no global keyboard
        side-effects, no Python-on-input-hot-path. Overlay shortcuts
        (Esc / R / F) are handled by the overlay's own Tk bindings,
        which work because the overlay grabs foreground focus.
        ...
        def _thread():
            try:
                ...
                MOD_ALT      = 0x0001
                MOD_CONTROL  = 0x0002
                MOD_NOREPEAT = 0x4000   # don't fire repeatedly when key is held
                WM_HOTKEY    = 0x0312
                ...
                if fs_vk is not None:
                    ...
                    if u32.RegisterHotKey(None, ID_FULLSCREEN, mod, fs_vk):
                        ...
                        print(f"[FreeShot] RegisterHotKey(fullscreen) failed",
                              file=sys.stderr)
                ...
                print(f"[FreeShot] hotkey: {e}", file=sys.stderr)
            finally:
                self._hotkey_tid = 0
